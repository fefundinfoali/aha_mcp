    ...
    switch (name) {
      ...
      case 'start_onboarding_wizard': {
        ...
        return {
          content: [{
            type: 'text',
            // CHANGED: Stronger instruction to force the display of data
            text: `
${table}
...
-->`
      ...
      case 'run_health_audit': {
        ...
        // Severity Helper: Critical=0, High=1, etc.
        const severityMap = { 'Critical': 0, 'High': 1, 'Medium': 2, 'Low': 3 };
        
        // Function to group violations by Reference Number
        function groupAndSort(violations, type) {
            ...
            // Convert to Array, Sort by Severity, Slice Top 10
            return Object.values(grouped)
                .sort((a, b) => a.MaxSeverityVal - b.MaxSeverityVal)
                .slice(0, 10)
                .map(item => {
                    // Construct Aha! URL (e.g. https://company.aha.io/initiatives/AI-123)
                    const urlType = type.toLowerCase() + 's'; // Initiative -> initiatives
                    const url = `https://${AHA_COMPANY}.aha.io/${urlType}/${item.Ref}`;

                    return {
                        Ref: `[${item.Ref}](${url})`, // Markdown Hyperlink
                    Name: item.Name,
        ...
        // Summary Matrix (Counts raw issues)
...
`;
        ...
        // Detailed Tables (Aggregated & Top 10)
